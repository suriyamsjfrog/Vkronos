...
module.exports.dropdownlist = async (email) => {
  try {
    let get_manager_data = await pool.query(
...
module.exports.csvfile = async (arr, startdate, enddate) => {
  ...
  arr.push(enddate);
  var queryText =
    "SELECT s.business_unit,s.working_day,s.employee_fullname,l.email,s.role_type,s.role_group,l.currentdate,l.logintime,l.logouttime,l.break FROM logintime as l,staff_admin as s WHERE l.email=s.email_address and l.username IN (" +
    params.join(",") +
    ")" +
    ...
    " and " +
    k +
    " order by l.username,l.currentdate";
